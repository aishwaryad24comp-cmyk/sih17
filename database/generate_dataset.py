...
UPDATE (this version): added the 6 outcome-tracking fields agreed with the team:
action_taken, final_outcome (Feature #19 - Historical Outcome Evidence),
project_budget (Feature #20 - Budget-Impact Estimator),
status, actual_end_date (Feature #22 - Completion Overview),
created_by (Feature #24 - Project Owner Access / "My Projects").
"""
...
# Outcome-tracking reference lists (new)
FINAL_OUTCOMES = ["On Time", "Delayed - Resolved", "Delayed - Unresolved", "Shelved"]
STATUSES = ["Ongoing", "Completed", "Stalled"]

# Synthetic seed officials, so created_by isn't blank for the bulk-generated rows.
# Real projects added later through the Add-Project form (Feature #21) will carry
# the actual logged-in official's user ID instead of one of these seed names.
SEED_OFFICIALS = [f"seed_officer_{i:03d}" for i in range(1, 41)]
...
for i in range(N):
    ...
    # project_budget (Feature #20): total project budget in INR. Compensation is
    # typically a fraction of total project cost for land-heavy infra (roads,
    # rail, irrigation) - use a random 15-35% share to back into a plausible total.
    compensation_share = np.random.uniform(0.15, 0.35)
    project_budget = int(round(compensation_assessed / compensation_share, -4))

    # status (Feature #22): derive from project age + delay behaviour rather than
    # pure randomness, so it's internally consistent with the other fields.
    project_age_days = (pd.Timestamp.today().normalize() - pd.Timestamp(notification_date)).days
    if project_age_days > 2.5 * 365 and not delayed:
        status = "Completed"
    elif project_age_days > 3.5 * 365 and delayed and risk_score < 0.55:
        # even a delayed project usually finishes eventually if risk wasn't severe
        status = np.random.choice(["Completed", "Ongoing"], p=[0.6, 0.4])
    elif delayed and risk_score > 0.45 and silence_days > 100:
        status = "Stalled"
    else:
        status = "Ongoing"

    # actual_end_date: only meaningful for Completed projects.
    # Pick a completion date sometime between notification and today (at least
    # 5 days ago), computed directly with pandas Timestamps to avoid edge cases
    # with Faker's relative-date parsing on very young projects.
    if status == "Completed":
        max_offset = max(project_age_days - 5, 5)
        days_ago = np.random.randint(5, max_offset + 1)
        actual_end_date = (pd.Timestamp.today().normalize() - pd.Timedelta(days=days_ago)).date().isoformat()
    else:
        actual_end_date = None

    # action_taken (Feature #19): whether corrective action was taken once a
    # project was flagged high-risk. Only meaningful for delayed projects.
    # More responsive stakeholders are more likely to have acted.
    if delayed:
        action_prob = {"High": 0.70, "Medium": 0.45, "Low": 0.20}[stakeholder_resp]
        action_taken = bool(np.random.rand() < action_prob)
    else:
        action_taken = None  # not applicable - project was never flagged high-risk

    # final_outcome (Feature #19): only assigned once a project has actually
    # concluded one way or another (Completed or Stalled). Ongoing projects
    # don't have a final outcome yet.
    if status == "Completed":
        final_outcome = "On Time" if not delayed else (
            "Delayed - Resolved" if action_taken else "Delayed - Unresolved"
        )
    elif status == "Stalled":
        # a stalled project with no action taken is the "shelving" case
        final_outcome = "Shelved" if not action_taken else "Delayed - Unresolved"
    else:
        final_outcome = None  # Ongoing - outcome not known yet

    # created_by (Feature #24): every bulk-seeded row gets a synthetic seed
    # official as its owner. Projects added later via the real Add-Project
    # form will overwrite this with the actual logged-in official's user ID.
    created_by = random.choice(SEED_OFFICIALS)
...
# sanity: new outcome fields distribution (helps confirm Feature #19/#22 have signal)
print("\nStatus distribution:")
print(df["status"].value_counts(normalize=True).round(2))
print("\nFinal outcome distribution (excludes Ongoing/None):")
print(df["final_outcome"].value_counts(normalize=True).round(2))
shelved_rate = (df["final_outcome"] == "Shelved").mean()
print(f"Overall shelving rate (for Feature #19 evidence): {shelved_rate:.1%}")
